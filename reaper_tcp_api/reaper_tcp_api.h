...
int call_AddMediaItemToTrack() {
    ...
    MediaTrack* tr;
    memcpy(&tr, &sendrecvbuf[buf_index], 8);
    buf_index += 8;

    MediaItem* retval = AddMediaItemToTrack(tr);
    buf_index = 0;

    memcpy(&sendrecvbuf[buf_index], &retval, 8);
    buf_index += 8;
...
int call_AddProjectMarker() {
    ...
    ReaProject* proj;
    memcpy(&proj, &sendrecvbuf[buf_index], 8);
    buf_index += 8;

    bool isrgn = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;

    double pos = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;

    double rgnend = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;

    const char* name = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(name);

    int wantidx = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int retval = AddProjectMarker(proj, isrgn, pos, rgnend, name, wantidx);
    buf_index = 0;

    memcpy(&sendrecvbuf[buf_index], &retval, 4);
    buf_index += 4;
...
int call_AddProjectMarker2() {
    ...
    int color = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_AddRemoveReaScript() {
    ...
    bool add = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;

    int sectionID = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    const char* scriptfn = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(scriptfn);

    bool commit = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_AddTakeToMediaItem() {
    ...
    MediaItem* item;
    memcpy(&item, &sendrecvbuf[buf_index], 8);
    buf_index += 8;
...
int call_AddTempoTimeSigMarker() {
    ...
    double timepos = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;

    double bpm = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;

    int timesig_num = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int timesig_denom = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    bool lineartempochange = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;

    bool retval = AddTempoTimeSigMarker(proj, timepos, bpm, timesig_num, timesig_denom, lineartempochange);
    buf_index = 0;

    sendrecvbuf[buf_index] = retval ? 1 : 0;
    buf_index += 1;
...
int call_adjustZoom() {
    ...
    double amt = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;

    int forceset = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    bool doupd = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;

    int centermode = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_APIExists() {
    ...
    const char* function_name = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(function_name);
...
int call_ApplyNudge() {
    ...
    ReaProject* project;
    memcpy(&project, &sendrecvbuf[buf_index], 8);
    buf_index += 8;

    int nudgeflag = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int nudgewhat = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int nudgeunits = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    double value = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;

    bool reverse = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;

    int copies = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_AudioAccessorValidateState() {
    ...
    memcpy(&accessor, &sendrecvbuf[buf_index], 8);
    buf_index += 8;
...
int call_BypassFxAllTracks() {
    ...
    int bypass = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_ColorToNative() {
    ...
    int r = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int g = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int b = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_CountEnvelopePoints() {
    ...
    memcpy(&envelope, &sendrecvbuf[buf_index], 8);
    buf_index += 8;
...
int call_CountSelectedTracks2() {
    ...
    bool wantmaster = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_CountTakeEnvelopes() {
    ...
    MediaItem_Take* take;
    memcpy(&take, &sendrecvbuf[buf_index], 8);
    buf_index += 8;
...
int call_CountTCPFXParms() {
    ...
    MediaTrack* track;
    memcpy(&track, &sendrecvbuf[buf_index], 8);
    buf_index += 8;
...
int call_CreateTrackSend() {
    ...
    MediaTrack* desttrInOptional;
    memcpy(&desttrInOptional, &sendrecvbuf[buf_index], 8);
    buf_index += 8;
...
int call_CSurf_FlushUndo() {
    ...
    bool force = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_CSurf_GetTouchState() {
    ...
    MediaTrack* trackid;
    memcpy(&trackid, &sendrecvbuf[buf_index], 8);
    buf_index += 8;

    int isPan = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_CSurf_NumTracks() {
    ...
    bool mcpView = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_CSurf_OnArrow() {
    ...
    int whichdir = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    bool wantzoom = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_CSurf_OnFwd() {
    ...
    int seekplay = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_CSurf_OnFXChange() {
    ...
    int en = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_CSurf_OnInputMonitorChange() {
    ...
    int monitor = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_CSurf_OnInputMonitorChangeEx() {
    ...
    bool allowgang = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_CSurf_OnMuteChange() {
    ...
    int mute = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_CSurf_OnPanChange() {
    ...
    double pan = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;

    bool relative = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_CSurf_OnPlayRateChange() {
    ...
    double playrate = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_CSurf_OnRecArmChange() {
    ...
    int recarm = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_CSurf_OnRecvPanChange() {
    ...
    int recv_index = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_CSurf_OnRecvVolumeChange() {
    ...
    double volume = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_CSurf_OnRewFwd() {
    ...
    int dir = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_CSurf_OnScroll() {
    ...
    int xdir = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int ydir = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_CSurf_OnSelectedChange() {
    ...
    int selected = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_CSurf_OnSendPanChange() {
    ...
    int send_index = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_CSurf_OnSoloChange() {
    ...
    int solo = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_CSurf_OnWidthChange() {
    ...
    double width = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_CSurf_SetAutoMode() {
    ...
    int mode = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    IReaperControlSurface* ignoresurf;
    memcpy(&ignoresurf, &sendrecvbuf[buf_index], 8);
    buf_index += 8;
...
int call_CSurf_SetPlayState() {
    ...
    bool play = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;

    bool pause = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;

    bool rec = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_CSurf_SetRepeatState() {
    ...
    bool rep = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_CSurf_SetSurfaceMute() {
    ...
    bool mute = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_CSurf_SetSurfaceRecArm() {
    ...
    bool recarm = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_CSurf_SetSurfaceSelected() {
    ...
    bool selected = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_CSurf_SetSurfaceSolo() {
    ...
    bool solo = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_CSurf_TrackFromID() {
    ...
    int idx = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_DB2SLIDER() {
    ...
    double x = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_DeleteEnvelopePointRange() {
    ...
    double time_start = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;

    double time_end = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_DeleteExtState() {
    ...
    const char* section = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(section);

    const char* key = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(key);

    bool persist = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_DeleteProjectMarker() {
    ...
    int markrgnindexnumber = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_DeleteProjectMarkerByIndex() {
    ...
    int markrgnidx = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_DeleteTempoTimeSigMarker() {
    ...
    int markerindex = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_DeleteTrackMediaItem() {
    ...
    MediaItem* it;
    memcpy(&it, &sendrecvbuf[buf_index], 8);
    buf_index += 8;
...
int call_Dock_UpdateDockID() {
    ...
    const char* ident_str = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(ident_str);

    int whichDock = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_DockWindowActivate() {
    ...
    HWND hwnd;
    memcpy(&hwnd, &sendrecvbuf[buf_index], 8);
    buf_index += 8;
...
int call_DockWindowAdd() {
    ...
    int pos = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    bool allowShow = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_DockWindowAddEx() {
    ...
    const char* identstr = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(identstr);
...
int call_EnumerateFiles() {
    ...
    const char* path = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(path);

    int fileindex = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    const char* retval = EnumerateFiles(path, fileindex);
    buf_index = 0;

    memcpy(&sendrecvbuf[buf_index], retval, 1 + strlen(retval));
    buf_index += 1 + strlen(retval);
...
int call_EnumerateSubdirectories() {
    ...
    int subdirindex = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_EnumPitchShiftSubModes() {
    ...
    int submode = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_EnumRegionRenderMatrix() {
    ...
    int regionindex = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int rendertrack = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_ExecProcess() {
    ...
    const char* cmdline = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(cmdline);

    int timeoutmsec = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_FindTempoTimeSigMarker() {
    ...
    double time = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_GetAllProjectPlayStates() {
    ...
    memcpy(&ignoreProject, &sendrecvbuf[buf_index], 8);
    buf_index += 8;
...
int call_GetCursorContext2() {
    ...
    bool want_last_valid = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_GetEnvelopeScalingMode() {
    ...
    TrackEnvelope* env;
    memcpy(&env, &sendrecvbuf[buf_index], 8);
    buf_index += 8;
...
int call_GetFreeDiskSpaceForRecordPath() {
    ...
    int pathidx = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_GetFXEnvelope() {
    ...
    int fxindex = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int parameterindex = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    bool create = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_GetInputChannelName() {
    ...
    int channelIndex = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_GetMediaItem() {
    ...
    int itemidx = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_GetMediaItemInfo_Value() {
    ...
    const char* parmname = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(parmname);
...
int call_GetMediaItemTake() {
    ...
    int tk = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_GetMediaSourceNumChannels() {
    ...
    PCM_source* source;
    memcpy(&source, &sendrecvbuf[buf_index], 8);
    buf_index += 8;
...
int call_GetMediaSourceParent() {
    ...
    PCM_source* src;
    memcpy(&src, &sendrecvbuf[buf_index], 8);
    buf_index += 8;
...
int call_GetProjectTimeOffset() {
    ...
    bool rndframe = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_GetSelectedMediaItem() {
    ...
    int selitem = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_GetSelectedTrack() {
    ...
    int seltrackidx = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_GetSetRepeat() {
    ...
    int val = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_GetTake() {
    ...
    int takeidx = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_GetTakeEnvelope() {
    ...
    int envidx = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_GetTakeEnvelopeByName() {
    ...
    const char* envname = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(envname);
...
int call_GetToggleCommandState() {
    ...
    int command_id = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_GetToggleCommandStateEx() {
    ...
    int section_id = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_GetTrack() {
    ...
    int trackidx = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_GetTrackEnvelopeByChunkName() {
    ...
    const char* cfgchunkname = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(cfgchunkname);
...
int call_GetTrackMIDINoteName() {
    ...
    int track = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int pitch = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int chan = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_GetTrackNumSends() {
    ...
    int category = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_GetTrackSendInfo_Value() {
    ...
    int sendidx = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_GoToMarker() {
    ...
    int marker_index = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    bool use_timeline_order = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_GoToRegion() {
    ...
    int region_index = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_GSC_mainwnd() {
    ...
    int t = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_Help_Set() {
    ...
    const char* helpstring = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(helpstring);

    bool is_temporary_help = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_InsertMedia() {
    ...
    const char* file = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(file);
...
int call_InsertMediaSection() {
    ...
    double startpct = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;

    double endpct = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;

    double pitchshift = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_InsertTrackAtIndex() {
    ...
    bool wantDefaults = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_IsMediaExtension() {
    ...
    const char* ext = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(ext);

    bool wantOthers = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_IsTrackVisible() {
    ...
    bool mixer = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_Loop_OnArrow() {
    ...
    int direction = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_Main_OnCommand() {
    ...
    int command = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int flag = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_Main_SaveProject() {
    ...
    bool forceSaveAsInOptional = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_Main_UpdateLoopInfo() {
    ...
    int ignoremask = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_Master_GetPlayRateAtTime() {
    ...
    double time_s = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_Master_NormalizePlayRate() {
    ...
    bool isnormalized = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_MB() {
    ...
    const char* msg = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(msg);

    const char* title = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(title);

    int type = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_MIDI_DeleteCC() {
    ...
    int ccidx = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_MIDI_DeleteEvt() {
    ...
    int evtidx = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_MIDI_DeleteNote() {
    ...
    int noteidx = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_MIDI_DeleteTextSysexEvt() {
    ...
    int textsyxevtidx = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_MIDI_EnumSelTextSysexEvts() {
    ...
    int textsyxidx = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_MIDI_GetPPQPos_EndOfMeasure() {
    ...
    double ppqpos = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_MIDI_GetPPQPosFromProjQN() {
    ...
    double projqn = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_MIDI_GetPPQPosFromProjTime() {
    ...
    double projtime = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_MIDI_InsertCC() {
    ...
    bool muted = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;

    double ppqpos = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;

    int chanmsg = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int chan = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int msg2 = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int msg3 = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_MIDI_InsertEvt() {
    ...
    const char* bytestr = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(bytestr);

    int bytestr_sz = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_MIDI_SelectAll() {
    ...
    bool select = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_MIDI_SetAllEvts() {
    ...
    const char* buf = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(buf);

    int buf_sz = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_MIDI_SetItemExtents() {
    ...
    double startQN = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;

    double endQN = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_MIDIEditor_GetMode() {
    ...
    HWND midieditor;
    memcpy(&midieditor, &sendrecvbuf[buf_index], 8);
    buf_index += 8;
...
int call_MIDIEditor_GetSetting_int() {
    ...
    const char* setting_desc = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(setting_desc);
...
int call_MIDIEditor_LastFocused_OnCommand() {
    ...
    bool islistviewcommand = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_MoveEditCursor() {
    ...
    double adjamt = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;

    bool dosel = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_MoveMediaItemToTrack() {
    ...
    MediaTrack* desttr;
    memcpy(&desttr, &sendrecvbuf[buf_index], 8);
    buf_index += 8;
...
int call_NamedCommandLookup() {
    ...
    const char* command_name = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(command_name);
...
int call_OpenColorThemeFile() {
    ...
    const char* fn = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(fn);
...
int call_OpenMediaExplorer() {
    ...
    const char* mediafn = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(mediafn);
...
int call_parse_timestr_len() {
    ...
    double offset = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;

    int modeoverride = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_parsepanstr() {
    ...
    const char* str = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(str);
...
int call_PCM_Sink_GetExtension() {
    ...
    const char* data = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(data);

    int data_sz = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_PCM_Sink_ShowConfig() {
    ...
    const char* cfg = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(cfg);

    int cfg_sz = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    HWND hwndParent;
    memcpy(&hwndParent, &sendrecvbuf[buf_index], 8);
    buf_index += 8;
...
int call_PCM_Source_CreateFromFile() {
    ...
    const char* filename = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(filename);
...
int call_PCM_Source_CreateFromFileEx() {
    ...
    bool forcenoMidiImp = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_PCM_Source_CreateFromType() {
    ...
    const char* sourcetype = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(sourcetype);
...
int call_PluginWantsAlwaysRunFx() {
    ...
    int amt = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_PreventUIRefresh() {
    ...
    int prevent_count = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_ReaScriptError() {
    ...
    const char* errmsg = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(errmsg);
...
int call_RenderFileSection() {
    ...
    const char* source_filename = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(source_filename);

    const char* target_filename = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(target_filename);

    double start_percent = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;

    double end_percent = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_ScaleFromEnvelopeMode() {
    ...
    int scaling_mode = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    double val = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_SetAutomationMode() {
    ...
    bool onlySel = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_SetCurrentBPM() {
    ...
    ReaProject* __proj;
    memcpy(&__proj, &sendrecvbuf[buf_index], 8);
    buf_index += 8;

    double bpm = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;

    bool wantUndo = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_SetCursorContext() {
    ...
    TrackEnvelope* envInOptional;
    memcpy(&envInOptional, &sendrecvbuf[buf_index], 8);
    buf_index += 8;
...
int call_SetEditCurPos() {
    ...
    bool moveview = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;

    bool seekplay = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_SetEnvelopeStateChunk() {
    ...
    bool isundoOptional = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_SetExtState() {
    ...
    const char* value = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(value);
...
int call_SetMediaItemInfo_Value() {
    ...
    double newvalue = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_SetMediaItemLength() {
    ...
    double length = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;

    bool refreshUI = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_SetMediaItemPosition() {
    ...
    double position = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_SetMIDIEditorGrid() {
    ...
    double division = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_SetMixerScroll() {
    ...
    memcpy(&leftmosttrack, &sendrecvbuf[buf_index], 8);
    buf_index += 8;
...
int call_SetMouseModifier() {
    ...
    const char* context = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(context);

    int modifier_flag = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    const char* action = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(action);
...
int call_SetProjectMarker4() {
    ...
    int flags = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_SetProjectMarkerByIndex() {
    ...
    int IDnumber = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_SetProjExtState() {
    ...
    const char* extname = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(extname);
...
int call_SetRegionRenderMatrix() {
    ...
    int addorremove = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_SetTakeStretchMarkerSlope() {
    ...
    double slope = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_SetTempoTimeSigMarker() {
    ...
    int ptidx = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    double timepos = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;

    int measurepos = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    double beatpos = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
    ...
    bool lineartempo = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_SetToggleCommandState() {
    ...
    int state = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_SetTrackSendUIPan() {
    ...
    int send_idx = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    double pan = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;

    int isend = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_SetTrackSendUIVol() {
    ...
    double vol = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_SLIDER2DB() {
    ...
    double y = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_SnapToGrid() {
    ...
    double time_pos = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_StuffMIDIMessage() {
    ...
    int msg1 = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_TakeFX_AddByName() {
    ...
    const char* fxname = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(fxname);

    int instantiate = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_TakeFX_EndParamEdit() {
    ...
    int fx = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int param = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_TakeFX_GetFloatingWindow() {
    ...
    int index = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_TakeFX_NavigatePresets() {
    ...
    int presetmove = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_TakeFX_SetEnabled() {
    ...
    bool enabled = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_TakeFX_SetOpen() {
    ...
    bool open = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_TakeFX_SetPinMappings() {
    ...
    int isOutput = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int pin = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int low32bits = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int hi32bits = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_TakeFX_SetPreset() {
    ...
    const char* presetname = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(presetname);
...
int call_TakeFX_Show() {
    ...
    int showFlag = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_TimeMap2_QNToTime() {
    ...
    double qn = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_TimeMap2_timeToQN() {
    ...
    double tpos = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;
...
int call_Track_GetPeakHoldDB() {
    ...
    int channel = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    bool clear = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_TrackCtl_SetToolTip() {
    ...
    const char* fmt = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(fmt);

    int xpos = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int ypos = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    bool topmost = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_TrackFX_AddByName() {
    ...
    bool recFX = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_TrackFX_GetByName() {
    ...
    bool instantiate = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_TrackFX_GetEQBandEnabled() {
    ...
    int fxidx = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int bandtype = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int bandidx = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_TrackFX_SetEQBandEnabled() {
    ...
    bool enable = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_TrackFX_SetEQParam() {
    ...
    int paramtype = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    double val = *(double*)&sendrecvbuf[buf_index];
    buf_index += 8;

    bool isnorm = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_TrackList_AdjustWindows() {
    ...
    bool isMinor = sendrecvbuf[buf_index] ? true : false;
    buf_index += 1;
...
int call_Undo_EndBlock() {
    ...
    const char* descchange = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(descchange);

    int extraflags = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_Undo_OnStateChangeEx() {
    ...
    int whichStates = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    int trackparm = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;
...
int call_ViewPrefs() {
    ...
    int page = *(int*)&sendrecvbuf[buf_index];
    buf_index += 4;

    const char* pageByName = &sendrecvbuf[buf_index];
    buf_index += 1 + strlen(pageByName);
